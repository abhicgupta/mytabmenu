...
class Database{
	 ...
	 * @return array
	 */
	private function db_fetch_row($result) {
	 ...
	 * @return array
	 */
	private function db_fetch_array($result, $flag=MYSQL_ASSOC) {
	 ...
	 * @return Integer
	 */
	private function db_num_rows($result) {
	 ...
	 * @return array
	 */
	public function func_query($query, $flag=MYSQL_ASSOC) {
		$result = false;
		if ($p_result  = self::db_query($query)) {
			while($arr = self::db_fetch_array($p_result,$flag))
			$result[]=$arr;
	...
	public function func_array2update ($tbl, $arr, $where = '') {
		...
		//echo "QUERY : [ UPDATE $tbl SET $r" . ($where ? " WHERE " . $where : "");
	...
	public function rowcount($query){
		...
		return self::db_num_rows($result);
	}
	...
	public function getColumns($tableName) {
		...
		//echo "Query : ".$columnTable;
		$columnResult = self::db_query($columnTable);	
		//echo 
		while($columnRow  = self::db_fetch_row($columnResult)) {
